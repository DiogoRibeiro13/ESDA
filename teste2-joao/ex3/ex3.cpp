...
string maxCollisions(HashTable &ht, vector<string> &v)
{
    ...
    vector<string> tabela = ht.getTable();
    bool at_least_one_found = false;
    int max = 0;
    string smax = tabela[0];

    for (size_t i = 0; i < v.size(); i++)
    {
        int collisions = 0;
        int index = ht.probingFunction(v[i], collisions);
        while (tabela[index] != v[i] && collisions < (int)v.size())
        {
            collisions++;
            index = ht.probingFunction(v[i], collisions);
            if (tabela[index] == v[i])
            {
