...
int swapHeadAndTail(list<int> *list_name)
{
    ...
    swap(list_name->front(), list_name->back());
    return 0;
}
...
{
    if (!list_name || list_name->empty() || k > (int)list_name->size() || k < 0)
        return -1;

    vector<int> to_reverse;

    while ((int)to_reverse.size() < k)
    {
        to_reverse.push_back(list_name->back());
        list_name->pop_back();
    }
