...
{
    if(list_name == nullptr || list_name->empty())
    {
        return -1;
    }

    auto temp = list_name->front();
...
{
    if(list_name == nullptr || list_name->empty() || k <= 0)
    {
        return -1;
    }

    auto back_element = list_name->back();
    auto front_element = list_name->back();
    int i = 0;

    //solução do joão é melhor
