...
{
    if(k > (int)vec.size() || k <= 0)
    {
        return 9999;
    }
 
    priority_queue<int, vector<int>, greater<int>> organized_pq;

    for(size_t pos = 0; pos < vec.size(); pos++)
    {
        organized_pq.push(vec[pos]);
    }
...
string largestPosElementString(string text, int k)
{
    ...
    priority_queue<string, vector<string>, compareMax> organized_string_pq;
    ...
    {
        organized_string_pq.push(aux);
    }

    if(k > (int)organized_string_pq.size() || k <= 0)
    {
        return error;
    }
    ...
    while(i < (k-1))
    {
        organized_string_pq.pop();
        i++;
    }

    return organized_string_pq.top();
}
