...
uint Tree::prodReplace(Node *tree_node)
{
    ...
    unsigned int left = prodReplace(tree_node->left);
    unsigned int right = prodReplace(tree_node->right);
    
    unsigned int old_value = tree_node->data;
    tree_node->data = left * right;
    
    return old_value * tree_node->data;
}

void Tree::printSibling(Node *search_node, Node *tree_node)
{
    if(search_node == nullptr || tree_node == nullptr)
    {
        return;
    }
    ...
    {
        if (tree_node->left->data == search_node->data)
        {
            cout << tree_node->right->data;
            return;
        }
        
        else if (tree_node->right->data == search_node->data)
        {
